...
from gomoku.strategy import Strategy, opponentOf, performMove, copyOfBoard
import time
import os
import sys
...
MOST_RECENT_HIGHLIGHT_COLOR = '\u001b[48;5;238m' 
...
SAVE_FILENAME = "gomoku_saved_game.txt"
BOARD_HISTORY = [] 
...
def main():
	...
	print(intro()), print(""), printAsciiTitleArt()
	...
	if useSavedGame:
		...
		boardDimension = input(f"AI constants are modifiable in the strategy.py file. (Developer Only). \nWhat is the dimension of the board? (Default is 13x13)\nEnter a single {RED_COLOR}ODD{NO_COLOR} number between 6-30:\t").strip()
		erasePreviousLines(2)
		if boardDimension.isdigit() and int(boardDimension) % 2 == 1 and 6 < int(boardDimension) < 30:
	...
	while not gameOver:
		...
		print("%s played in spot %s%s" % (nameOfCurrentPlayer, moveFormatted, timeTakenOutputStr))
